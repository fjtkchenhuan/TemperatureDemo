...
{
     return MLX90640_I2CRead(fd, 0x2400, 832, eeData);
}
...
int MLX90640_GetFrameData(int fd, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        error = MLX90640_I2CRead(fd, 0x8000, 1, &statusRegister);//A new data is available in RAM or not
        if(error != 0)
        {
            return error;
        }    
    ...
    { 
        error = MLX90640_I2CWrite(fd, 0x8000, 0x0030);//?
        if(error == -1)
        {
            return error;
        }
            
        error = MLX90640_I2CRead(fd, 0x0400, 832, frameData); 
        if(error != 0)
        {
            return error;
        }
                   
        error = MLX90640_I2CRead(fd, 0x8000, 1, &statusRegister);
        if(error != 0)
        {
            return error;
        }    
    ...
    error = MLX90640_I2CRead(fd, 0x800D, 1, &controlRegister1);
...
int MLX90640_SetResolution(int fd, uint8_t resolution)
{
    ...
    if(error == 0)
    {
        ...
        error = MLX90640_I2CWrite(fd, 0x800D, value);        
    }    
...
//------------------------------------------------------------------------------
int MLX90640_SetInit(int fd,int mode)
{
    uint16_t value;
    int error;
    if(0 == mode)
		value = 0x1901;//Rate2HZ
    else if(1 == mode)
		value = 0x1981;//Rate4HZ
    else if(2 == mode)
		value = 0x1a01;//Rate8HZ
    else
		value = 0x1a81;//Rate16HZ
    error = MLX90640_I2CWrite(fd, 0x800D, value);           
    return error;
